...
def openai_tools_to_definitions(tools):
    """Convert OpenAI's wire shape for tools into `gen_ai.tool.definitions` items.

    OpenAI nests the function under a `function` key; the conventions define a
...
def run_chat_reference(client):
    ...
    with _reference_tracer.start_as_current_span("chat gpt-4o-mini", kind=SpanKind.CLIENT, attributes=span_attributes) as span:
...
def run_chat_streaming_reference(client):
    ...
    with _reference_tracer.start_as_current_span("chat gpt-4o-mini", kind=SpanKind.CLIENT, attributes=span_attributes_2) as span:
        span.set_attribute(
            "gen_ai.input.messages",
            json.dumps(
...
def run_chat_tool_call_reference(client):
    ...
    with _reference_tracer.start_as_current_span("chat gpt-4o-mini", kind=SpanKind.CLIENT, attributes=span_attributes_3) as span:
        span.set_attribute("gen_ai.tool.definitions", json.dumps(openai_tools_to_definitions(tools)))
        resp = client.chat.completions.create(
            model=request_model,
...
def run_chat_with_document_input_reference(client):
    ...
    with _reference_tracer.start_as_current_span("chat gpt-4o-mini", kind=SpanKind.CLIENT, attributes=span_attributes_doc) as span:
        span.set_attribute("gen_ai.input.messages", input_messages)
        resp = client.chat.completions.create(
            model=request_model,
            messages=messages,
...
def responses_finish_reason(fetched):
    """Map a Responses `status` to a `gen_ai.response.finish_reasons` value.

    A completed generation maps to its stop reason, an incomplete one to why it
    was cut short, and a failed or cancelled generation to `error`. On a fetch
    this conveys the outcome of the ORIGINAL generation, not of the fetch, which
...
def _emit_fetch_response_span(client, response_id, starting_after=None):
    ...
    if port is not None:
        span_attributes["server.port"] = port
    with _reference_tracer.start_as_current_span("fetch_response", kind=SpanKind.CLIENT, attributes=span_attributes) as span:
        if starting_after is not None:
