...
# Generous: a cold runtime with instrumentation enabled is slow to reach its
...
def _serve_and_drive(command: Sequence[str]) -> int:
    ...
    # real server process, directly or indirectly, and waits for it, so killing
    # the launcher alone would leave the server running.
    reservation.close()
